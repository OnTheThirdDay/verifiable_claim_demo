...
class Request {
  ...
  static Future<http.Response> httpGet(String url, Map<String, dynamic> body) async {
    if (body == null) {
      return await client.get(Uri.tryParse(url)!);
    } else {
      var paramString = '?';
      body.forEach((k, v) {
        if (v != null) {
      ...
      try {
        var result = await client.get(
          Uri.tryParse(url + paramString.substring(0, paramString.length - 1))!,
        );
        return result;
      } catch (_) {
  ...
  static Future<http.Response> httpPost(String url, Map<String, dynamic> body) async {
    if (body == null) {
      try {
        var result = await client.post(Uri.tryParse(url)!);
        return result;
      } catch (_) {
        throw NetworkException();
      }
      ...
      try {
        var result = await client.post(Uri.tryParse(url)!, body: paramMap);
        return result;
      } catch (_) {
        throw NetworkException();
      }
